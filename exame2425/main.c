...
void datasProjetosGestor(int idGestor, pproj lista, pgest vetor, int dim) {
    struct data inicio_old;
    struct data inicio_recente;

    pproj aux = lista;
    int encontrou = 0;
    while (aux != NULL) {
        if (idGestor == aux->p->id && aux->p->total >= 2) {
            if (!encontrou) {
                inicio_old = aux->inicio;
                inicio_recente = aux->inicio;
                encontrou = 1;
            } else {
                // mais antigo
                if (aux->inicio.ano < inicio_old.ano || (
                        aux->inicio.ano == inicio_old.ano && aux->inicio.mes < inicio_old.mes) ||
                    (aux->inicio.ano == inicio_old.ano && aux->inicio.mes == inicio_old.mes && aux->inicio.dia <
                     inicio_old.dia)) {
                    inicio_old = aux->inicio;
                }
                // mais recente
                if (aux->inicio.ano > inicio_recente.ano ||
                    (aux->inicio.ano == inicio_recente.ano && aux->inicio.mes > inicio_recente.mes) ||
                    (aux->inicio.ano == inicio_recente.ano && aux->inicio.mes == inicio_recente.mes && aux->inicio.dia >
                     inicio_recente.dia)) {
                    inicio_recente = aux->inicio;
                }
            }
    ...
    if (!encontrou)
        ...
        printf("Mais antigo: %02d/%02d/%04d\nMais recente: %02d/%02d/%04d\n",
               inicio_old.dia, inicio_old.mes, inicio_old.ano,
               inicio_recente.dia, inicio_recente.mes, inicio_recente.ano);
...
pgest eliminarGestor(int idElim, pproj lista, pgest vetor, int *dim){
    pproj aux = lista;
    // se só tem 1 gestor, não faz nada
    if(*dim == 1) return vetor;

    // procurar o gestor a eliminar no vetor
    int idxElim = -1;
    for(int i = 0; i < *dim; i++){
        if(vetor[i].id == idElim){
            idxElim = i;
            break;
        }
    }
    ...
    if(idxElim == -1) return vetor;

    int idxMin = -1;
    for(int i = 0; i < *dim; i++){
        if(i == idxElim) continue; // ignorar o que vai ser eliminado
        if(idxMin == -1 || vetor[i].total < vetor[idxMin].total){
            idxMin = i;
        }
    }
    ...
    while(aux != NULL){
        if (aux->p->id == idElim) {
            aux->p = &vetor[idxMin];  // aponta para o novo gestor
            vetor[idxMin].total++;    // incrementa o total do novo gestor
        }
        aux = aux->prox;
    }

    // shift para remover o gestor eliminado
    for(int i = idxElim; i < *dim - 1; i++){
        vetor[i] = vetor[i+1];
    }
    (*dim)--;

    // realloc para reduzir o vetor
    pgest temp = realloc(vetor, *dim * sizeof(gestor));
    if(temp == NULL) return vetor; // problema de memória
    vetor = temp;
